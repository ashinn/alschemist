    ...
    (define (run-tests)
      (parameterize ((finance-offline-data? #t))
        (test-group "(chibi finance asset)"
          ...
          (let* ((aapl (make-stock 'AAPL 100. 'USD))
                 (asset (make-asset "Apple" 100 aapl)))
            (test 10000. (asset-value-in asset 'USD))
            (stock-price-set! aapl 200.)
            (test 20000. (asset-value-in asset 'USD))
            (test 10000. (asset-gain asset)))
          (let* ((aapl (make-stock 'AAPL 200. 'USD))
                 (asset (make-asset "Apple" 100 aapl)))
            (test 20000. (asset-value-in asset 'USD))
            (stock-price-set! aapl 100.)
            (test 10000. (asset-value-in asset 'USD))
            (test -10000. (asset-gain asset)))
          (let* ((aapl (make-stock 'AAPL 100. 'USD))
                 (asset (make-asset "Apple" 100 aapl)))
            ...
            (test 20000. (asset-value-in asset 'USD))
            (asset-inc! asset 100)
            (test 40000. (asset-value-in asset 'USD))
            ;; we bought after the stock increase
            (test 10000. (asset-gain asset))
            ))))))
