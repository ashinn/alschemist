  ...
  (begin
    (define (run-tests)
      ...
      (let ()
        ...
        (define (relu t)
          (lambda (weights)
            (let ((res ((linear t) weights)))
              (.rectify res))))
        (let*-duals ((w0 (tensor '((2. 1. -3.1) (3.7 4. 6.1))))
                     (w1 (tensor '((0.) (0.))))
                     (x (tensor '((1.3 .4 3.3))))
                     (y ((relu x) (list w0 w1)))
                     (loss (.mean (.square y)))
                     (grads (gradient loss (list w0 w1))))
          (test 352.1857604980469 (dual-value loss))
          (test-array (tensor '((0. 26.54) (0. 26.54))) (dual-value y))
          (test-array (tensor '((0. 0. 0.)
                                (34.502 10.616 87.582)))
            (first grads))
          ;; the gradient of the constant
          (test-array (tensor '((13.27) (13.27))) (second grads)))
        (let*-duals ((x (tensor '((2. 1. -3.1) (3.7 4. 6.1))))
          ...
          (test-array (tensor '((0. 23.34) (0. 23.34))) (dual-value y))
          (test 272.3778 (dual-value loss))
          (test-array (tensor '((0. 23.34) (0. 23.34))) (dual-value y))
          (test-array (tensor '((0. -0. 0.)
                                (30.342 -9.336 77.022)))
            (first grads))
          ;; the gradient of the constant
          (test-array (tensor '((11.67) (11.67))) (second grads))))
      ;; broadcasting
