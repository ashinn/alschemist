...
(define (init-weights shapes . o)
  (let ((storage (if (pair? o) (car o) generic-storage-class))
        (random-source (if (and (pair? o) (pair? (cdr o)))
                           (cadr o)
                           default-random-source)))
    (map (lambda (s) (init-shape s storage random-source)) shapes)))

(define (init-shape shape . o)
  (let ((storage (if (pair? o) (car o) generic-storage-class))
        (random-source (if (and (pair? o) (pair? (cdr o)))
                           (cadr o)
                           default-random-source)))
    (if (= 1 (length shape))
        (zeros (make-interval (vector (first shape))) storage)
        (random-array 0. (/ 2. (second shape)) shape storage random-source))))
  ...
  (let ((storage (if (pair? o) (car o) generic-storage-class))
    ...
    (specialized-array-reshape
     (make-specialized-array-from-data
      (random-sample (fold * 1 shape)
                     (normal-distribution mean variance random-source))
      storage)
